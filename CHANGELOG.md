...
- Updated library versions in pom.xml
- Build update.
- Increase Dukascopy rate limit as still being throttled even at 0.5/s
- Updated to the latest OSS standards framework and spring version.  Enforced Java 25, updated to Jackson 3, refactored cache statistics.  Lowered rate limiter again to 0.5/s as sustained loads cause high back offs from Dukascopy.
- Lime Versions: Update. (#2)

## 4.0.2

- Update documentation for next release.
- Reduced rate to under 1ps.  Updated rate limit to 503 as 500 doesn't work with live.
- Update doco for version change.
- Spelling fixes

## 4.0.1

- Add name for OSS deployment standards.
- Update test for no zero bars.  Update cache logging.
- Market status to handle edge conditions on weekends.  Updated tests for the removal of more paths.  Updated tests to handle empty lists of found paths.  Tightened logging.
- Added MarketStatus which allows check for FX market open or closed.  Used to reduce calls for zero return files from Dukascopy.  Updated tests for filtered paths.
- Rename workflows.
- feature/library-and-rate-limit-fixes (#1)
- GW-506 Manual version update after final commit release step failed
- Version update

## 3.0.0

- GW-506 Correct default branch
- GW-506 Deprecation warning fixes.
- GW-506 Vulnerability fix.
- GW-506 Upgrade to the latest os standards version.  Switch to JDK minimum and spring 6 support.  Updates for GitHub conversion.

## 2.1.3

- Updated TradingInputJsonStreams to support a visitor when restoring from json data.
- Updated readme by moving changes list to end.

## 2.1.2

- Corrected javadoc.
- Updated readme version
- Added example spring configuration.
- Readme.md edited online with Bitbucket

## 2.1.1

- Updated to the latest libraries and OSS standards.

## 2.1.0

- Tests to cover stream changes.  Some logging to help debug search calls.
- Moved generic stream classes into trading.model.streams package.  There are some minor API changes such as adding IOException to close methods where missed.  Added generic visitor interface.  Backwards and forwards search streams to top level.  Generalised the search streams.

## 2.0.5

- Corrected bug in TradingInputJsonStreams where hasNext may skip records depending on call pattern.  Fix ported from closed source.

## 2.0.4

- Corrected bug when using aggregate bar searching (forward or backwards, n periods) where a duplicate bar may appear in the stream.  This was due to the iterative search using an inclusive end instant to duplicate a bar.  Corrected so that the bar aggregators adjust the end criteria to be exlusive as expected.  Updated documentation to note that the end time should be exclusive.
- Copyright update.
